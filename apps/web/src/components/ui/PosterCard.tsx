...
interface PosterCardProps {
  ...
  height?: number;
  className?: string;
  /** translateZ depth in px (20–260), mapped from index to depth layer */
  depth?: number;
  showInfo?: boolean;
  onClick?: () => void;
}
...
/**
 * Glassmorphic poster card with:
 * - GSAP-driven magnetic hover (pointer → spring tilt)
 * - Warm cursor-tracking glow overlay
 * - BlurUpImage for progressive loading
 * - Depth-proportional shadow
 */
export default function PosterCard({
  ...
  /* ── Magnet hover: pointer position → GSAP spring tilt ── */
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    // ±9° tilt per axis, 1.04x scale lift
    ...
    // Move warm glow to cursor position
    if (glowRef.current) {
      glowRef.current.style.opacity = "1";
      glowRef.current.style.background = `radial-gradient(circle at ${
        (nx + 0.5) * 100
      }% ${(ny + 0.5) * 100}%, rgba(232,168,56,0.18) 0%, transparent 60%)`;
    }
  }, []);
  ...
  // Depth-proportional shadow: deeper cards cast longer shadows
  ...
  return (
    ...
    >
      ...
      {/* Glass info footer — slides up on hover */}
      {showInfo && (
