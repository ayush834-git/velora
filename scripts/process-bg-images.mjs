...
for (const [src, out] of MAP) {
  ...
  await sharp(src)
    .extract({ left: 0, top: 0,
      width:  Math.floor(meta.width  * 0.93),  // remove 7% right (watermark)
      height: Math.floor(meta.height * 0.91)   // remove 9% bottom (watermark)
    })
    .blur(10)                    // gaussian — removes banding, enables parallax blend
    .avif({ quality: 72 })
    .toFile(path.join(OUT, out));
  
  const stat = fs.statSync(path.join(OUT, out));
